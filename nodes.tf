...
resource "local_sensitive_file" "pem_file" {
  filename        = pathexpand("~/.ssh/eks-monesh.pem")
  file_permission = "600"
...
resource "aws_security_group" "node_security_group" {
  name        = "MoneshNodeSecurityGroupIngress"
...
resource "aws_vpc_security_group_ingress_rule" "node_security_group_from_control_plane_ingress" {
  ...
  security_group_id            = aws_security_group.node_security_group.id
  referenced_security_group_id = aws_eks_cluster.monesh_eks.vpc_config[0].cluster_security_group_id
  from_port                    = 1025
  to_port                      = 65535
  ip_protocol                  = "TCP"
}
